...
from app.api.deps import DBSession, UserRepo, LogRepo, AdminUser, ActiveUser, CurrentUser, capitalize_full_name
...
async def update_current_user(
    ...
    await log_repo.write_log(
        action_type="user_self_update",
        user_id=current_user.user_id,
        entity_id=current_user.user_id,
        details=f"Updated profile fields: {', '.join(update_data.keys())}",
    )
...
async def update_user(
    ...
    await log_repo.write_log(
        action_type="user_admin_update",
        user_id=current_user.user_id,
        entity_id=user_id,
        details=f"Admin updated user fields: {', '.join(update_data.keys())}",
    )
...
async def activate_user(user_id: int, request: Request, user_repo: UserRepo, log_repo: LogRepo, current_user: AdminUser) -> UserResponse:
...
async def delete_user(user_id: int, user_repo: UserRepo, log_repo: LogRepo, current_user: AdminUser) -> None:
    """
