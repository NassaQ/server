...
class Users(Base):
    ...
    role: Mapped[Optional["Roles"]] = relationship(
        "Roles", back_populates="Users", foreign_keys=[role_id], lazy="selectin"
    )
    Folders: Mapped[list["Folders"]] = relationship(
        "Folders", back_populates="created_by_user"
    )
    Individual_Permissions: Mapped[list["IndividualPermissions"]] = relationship(
        "IndividualPermissions", back_populates="user"
    )
    Logs: Mapped[list["Logs"]] = relationship("Logs", back_populates="user")
    Documents: Mapped[list["Documents"]] = relationship(
        "Documents", back_populates="uploaded_by_user"
    )
...
class Folders(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["created_by_user_id"], ["Users.user_id"], name="FK_Folder_Creator"
        ),
        ForeignKeyConstraint(
            ["parent_folder_id"], ["Folders.folder_id"], name="FK_Folder_Parent"
        ),
    ...
    parent_folder: Mapped[Optional["Folders"]] = relationship(
        "Folders", remote_side=[folder_id], back_populates="parent_folder_reverse"
    )
    parent_folder_reverse: Mapped[list["Folders"]] = relationship(
        "Folders", remote_side=[parent_folder_id], back_populates="parent_folder"
    )
    Documents: Mapped[list["Documents"]] = relationship(
        "Documents", back_populates="folder"
    )
...
class Documents(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["folder_id"], ["Folders.folder_id"], name="FK_Doc_Folder"
        ),
        ForeignKeyConstraint(
            ["uploaded_by_user_id"], ["Users.user_id"], name="FK_Doc_Uploader"
        ),
    ...
    filename: Mapped[str] = mapped_column(
        Unicode(255, "SQL_Latin1_General_CP1_CI_AS"), nullable=False
    )
    ...
    azure_blob_path: Mapped[str] = mapped_column(
        Unicode(255, "SQL_Latin1_General_CP1_CI_AS"), nullable=False
    )
    mongo_doc_id: Mapped[str] = mapped_column(
        String(36, "SQL_Latin1_General_CP1_CI_AS"), nullable=False
    )
    ...
    uploaded_by_user: Mapped["Users"] = relationship(
        "Users", back_populates="Documents"
    )
    Processing_Status: Mapped[list["ProcessingStatus"]] = relationship(
        "ProcessingStatus", back_populates="doc"
    )
